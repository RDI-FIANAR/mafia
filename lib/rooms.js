...
module.exports = {
  ...
  joinRoom: function(id, password, socket) {
    ...
    if(room.players.length >= room.size) {
      ...
      if(!room.lastFullNotice || room.lastFullNotice < (d - 60*1000)) {
        ...
        room.broadcast('chatMessage', {message: '<span class="glyphicon glyphicon-user"></span> <strong>Quelqu\'un</strong> voudrait rejoindre la partie. Pouvez vous ajouter plus de places?'});
      }
      return;
    }
    ...
    room.broadcast('chatMessage', {message: '<span class="glyphicon glyphicon-ok-circle"></span> <strong>' + socket.username + '</strong> a rejoint la partie'});
  ...
  leaveRoom: function(socket) {
    ...
    if(!room.started && socket.username) {
      room.broadcast('chatMessage', {message: '<span class="glyphicon glyphicon-remove-circle"></span> <strong>' + socket.username + '</strong> a quitté la partie'});
    }
